...
import torch
from utils.tools import process_f0
...
def dataio_prep(hparams):
    ...
    def audio_pipeline_s1(s1_wav):
        ...
        aux_name = os.path.basename(s1_wav).split(".")[0] + ".npy"
        file_name = os.path.dirname(s1_wav)
        f0_dir = os.path.join(file_name, "f0", aux_name)
        mel_dir = os.path.join(file_name, "mel", aux_name)
        # pitch_dir = os.path.join(file_name, "pitch", aux_name)

        spec = torch.Tensor(np.load(mel_dir))
        energy = (spec.exp() ** 2).sum(-1).sqrt()
        # pitch = torch.Tensor(np.load(pitch_dir))
        pitch, uv = process_f0(np.load(f0_dir), hparams)
        pitch = uv = energy
        return (s1_sig, spec, energy, pitch, uv)

    @sb.utils.data_pipeline.takes("s2_wav")
    @sb.utils.data_pipeline.provides("s2_sig")
    def audio_pipeline_s2(s2_wav):
        s2_sig = sb.dataio.dataio.read_audio(s2_wav)
        aux_name = os.path.basename(s2_wav).split(".")[0] + ".npy"
        file_name = os.path.dirname(s2_wav)
        f0_dir = os.path.join(file_name, "f0", aux_name)
        mel_dir = os.path.join(file_name, "mel", aux_name)
        # pitch_dir = os.path.join(file_name, "pitch", aux_name)
        ...
        pitch, uv = process_f0(np.load(f0_dir), hparams)
        pitch = uv = energy
        return (s2_sig, spec, energy, pitch, uv)
    ...
    if hparams["num_spks"] == 3:
        ...
        def audio_pipeline_s3(s3_wav):
            ...
            aux_name = os.path.basename(s3_wav).split(".")[0] + ".npy"
            file_name = os.path.dirname(s3_wav)
            f0_dir = os.path.join(file_name, "f0", aux_name)
            mel_dir = os.path.join(file_name, "mel", aux_name)
            # pitch_dir = os.path.join(file_name, "pitch", aux_name)
            ...
            pitch, uv = process_f0(np.load(f0_dir), hparams)
            pitch = uv = energy
            return (s3_sig, spec, energy, pitch, uv)

...
class Dual_Path_Model(nn.Module):
    ...
    def defusing_func(self, x_t):
        x_t = self.norm(x_t)
        # [B, N, L]
        x_t = self.conv1d(x_t)
        if self.use_global_pos_enc:
            x_t = self.pos_enc(x_t.transpose(1, -1)).transpose(1, -1) + x_t * (
                    x_t.size(1) ** 0.5
            )
        # [B, N, K, S]
        x_t, _ = self._Segmentation(x_t, self.K)
        return x_t
    ...
    def forward(self, x, x_t=None):
        ...
        if x_t != None:
            x_t = torch.cat(
                [self.defusing_func(x_t[i]) for i in range(self.num_spks)],
                dim=1
            )
            x = x + x_t

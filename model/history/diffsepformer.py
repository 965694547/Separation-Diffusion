...
class Conv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, dilation=1):
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, dilation=dilation, padding=self.padding)
...
def DiffNet(hparams):
    ...
    '''
    hparams["Conv_0"] = Conv(in_channels=hparams['num_spks'] + 1, out_channels=1)
    hparams['modules']['conv_0'] = hparams['Conv_0']
    hparams["Conv_1"] = Conv(in_channels=hparams['num_spks'] + 1, out_channels=1)
    hparams['modules']['conv_1'] = hparams['Conv_1']
    if hparams['num_spks']:
        hparams["Conv_2"] = Conv(in_channels=hparams['num_spks'] + 1, out_channels=1)
        hparams['modules']['conv_2'] = hparams['Conv_2']
    '''
...
class DiffSepformer(sb.Brain):
    def model(self, mix, diffusion_steps=None, input_targets=None):
        ...
        #sep_h[0] = self.hparams.Conv_0(torch.cat([sep_h[0].unsqueeze(0), input_targets], dim=0)).squeeze(0)
        #sep_h[1] = self.hparams.Conv_1(torch.cat([sep_h[1].unsqueeze(0), input_targets], dim=0)).squeeze(0)
        #if self.hparams.num_spks == 3:
        #    sep_h[2] = self.hparams.Conv_2(torch.cat([sep_h[2].unsqueeze(0), input_targets], dim=0)).squeeze(0)
    ...
    def compute_forward(self, mix, targets, stage, noise=None):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            est_source = self.model(mix, diffusion_steps.view(self.hparams.batch_size, 1), input_targets)
            # Gauss Noise
            self.transformer_predictions = transformer_source(diffusion_steps - 1, self.hparams.diffusion_hyperparams, est_source, z=Gauss)
            self.transformer_targets = transformer_source(diffusion_steps - 1, self.hparams.diffusion_hyperparams, targets, z=Gauss)
    ...
    def fit_batch(self, batch):
        ...
        if self.auto_mix_prec:
            with autocast():
                ...
                loss += self.compute_diff_loss(self.transformer_predictions, self.transformer_targets)
